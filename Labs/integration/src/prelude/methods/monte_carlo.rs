...
fn value<F: Fn(f64) -> f64>(fun: &Function<F>, i_sample: &[f64]) -> f64 {
    fun.h() * i_sample.iter().map(|&i| fun.f(i)).sum::<f64>()
}

fn error<F: Fn(f64) -> f64>(fun: &Function<F>, i_sample: &[f64]) -> f64 {
...
pub fn monte_carlo<F: Fn(f64) -> f64>(fun: &Function<F>) -> IntegralResult {
